/*
* Autor : Jeon-Kyeong Cha (Korea Electronics Technology Institute)
* This implementation of VideoSource allows to use still images as a source for the video input
* The code is implemented to support RTL design verification as test bench
*
* INSTALLATION :
...
void read_directory() {
	...
	handle = _findfirst(".\\IMG\\*.color.png", &fd);

	if (handle == -1) {
		cout << "There were no files !" << endl;
		return;
	}
	...
	while (result != -1) 
	{
		...
		fileName.push_back(fd.name);
		//cout << "fileName[" << i << "]: " << fileName[i] << endl; i++;
		result = _findnext(handle, &fd);
	}
	for (int i=0; i<1000; i++) cout << "fileName[" << i << "]: " << fileName[i] << endl; i++;
...
void VideoSource::GetAndFillFrameBWandRGB(Image<byte> &imBW, Image<Rgb<byte> > &imRGB)
{
	...
	*/
	Mat frame;
	string srcPath = ""; // . / IMG / frame - 000000.color.png";

	if (frameCnt == 1000) {
		frameCnt = 0;
	}

	srcPath = "./IMG/" + fileName[frameCnt];
	
	frame = imread(srcPath, CV_LOAD_IMAGE_COLOR);
